...
{
    var local:{sprite:FlxSprite, video:FlxVideo, tag:String} =
    {
        sprite: null,
...
function stopVideoSprite(tag:String):Void
{
	for (videoSprite in global)
	{
		if (videoSprite.tag == tag)
		{
			videoSprite.sprite.destroy();

	   	videoSprite.video.dispose();
		}
	}
}
...
createGlobalCallback("stopVideoSprite", stopVideoSprite);
